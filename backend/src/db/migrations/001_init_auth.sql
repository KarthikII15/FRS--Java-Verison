...
create table if not exists ivis_user (
...
create table if not exists ivis_tenant (
  pk_tenant_id bigserial primary key,
  tenant_name varchar(200) not null
);

create table if not exists ivis_customer (
  pk_customer_id bigserial primary key,
  customer_name varchar(200) not null,
  fk_tenant_id bigint not null references ivis_tenant(pk_tenant_id)
);

create table if not exists ivis_site (
  pk_site_id bigserial primary key,
  site_name varchar(200) not null,
  fk_customer_id bigint not null references ivis_customer(pk_customer_id)
);

create table if not exists ivis_unit (
  pk_unit_id bigserial primary key,
  unit_name varchar(200) not null,
  fk_site_id bigint not null references ivis_site(pk_site_id)
);

create table if not exists ivis_tenant_user_map (
  fk_user_id bigint not null references ivis_user(pk_user_id),
  fk_tenant_id bigint not null references ivis_tenant(pk_tenant_id),
  primary key (fk_user_id, fk_tenant_id)
);

create table if not exists ivis_customer_user_map (
  fk_user_id bigint not null references ivis_user(pk_user_id),
  fk_customer_id bigint not null references ivis_customer(pk_customer_id),
  primary key (fk_user_id, fk_customer_id)
);

create table if not exists ivis_user_membership (
...
create index if not exists idx_membership_user on ivis_user_membership(fk_user_id);
create index if not exists idx_membership_scope on ivis_user_membership(tenant_id, customer_id, site_id, unit_id);
create unique index if not exists uq_membership_user_scope_role
  on ivis_user_membership(fk_user_id, role, tenant_id, customer_id, site_id, unit_id);
