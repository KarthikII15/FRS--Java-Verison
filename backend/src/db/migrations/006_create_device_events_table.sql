...
CREATE TABLE IF NOT EXISTS device_events (
    ...
    frame_url TEXT,
    employee_id UUID REFERENCES employees(pk_employee_id), -- If matched
...
CREATE TABLE device_events_2024_03 PARTITION OF device_events
    FOR VALUES FROM ('2024-03-01') TO ('2024-04-01');

-- Indexes
CREATE INDEX idx_device_events_device_time ON device_events(fk_device_id, occurred_at DESC);
CREATE INDEX idx_device_events_unprocessed ON device_events(processing_status) WHERE processing_status = 'pending';
CREATE INDEX idx_device_events_type_time ON device_events(event_type, occurred_at DESC);
CREATE INDEX idx_device_events_face_embedding ON device_events USING ivfflat (detected_face_embedding vector_cosine_ops)
