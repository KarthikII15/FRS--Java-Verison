...
/**
 * Auto-provision a Keycloak user into ivis_user on first login.
 *
 ...
 *  3. If neither found                           →  create new ivis_user record
 *
 * @param {object} jwtPayload  Decoded Keycloak JWT
 * @returns {Promise<object>}  The ivis_user row
 */
export async function provisionKeycloakUser(jwtPayload) {
    ...
    if (existingUser) {
        ...
        await query(
            `UPDATE ivis_user SET keycloak_sub = $1 WHERE pk_user_id = $2`,
    ...
    const result = await query(
        `INSERT INTO ivis_user (email, username, role, keycloak_sub, fk_user_type_id, password_hash)
     VALUES ($1, $2, $3, $4, 1, '')
